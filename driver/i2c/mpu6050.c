...
bool mpu6050_init(void)
{
    ...
    uint8_t whoami;
    i2c_recv(MPU6050_ADDR, WHO_AM_I_REG, &whoami, 1);
    LCD_ShowString(0, 0, 16, 16, 16, (uint8_t*)whoami);
    ...
    // reset the device
    data = 0;
    i2c_send(MPU6050_ADDR, PWR_MGMT_1_REG, &data, 1);

    Delay_Ms(100);

    // set the sample rate to 1kHz
    data = 0x07;
    i2c_send(MPU6050_ADDR, SMPLRT_DIV_REG, &data, 1);

    // set the accelerometer configuration to 2g
    data = 0x00;
    i2c_send(MPU6050_ADDR, ACCEL_CONFIG_REG, &data, 1);

    // set the gyro configuration to 250deg/s
    data = 0x00;
    i2c_send(MPU6050_ADDR, GYRO_CONFIG_REG, &data, 1);
...
void mpu6050_read_accel(mpu6050_accel_t *accel)
{
    ...
    i2c_recv(MPU6050_ADDR, ACCEL_XOUT_H_REG, raw_data, 6);
...
void mpu6050_read_gyro(mpu6050_gyro_t *gyro)
{
    ...
    i2c_recv(MPU6050_ADDR, GYRO_XOUT_H_REG, raw_data, 6);
...
float mpu6050_read_temper(void)
{
    ...
    i2c_recv(MPU6050_ADDR, TEMP_OUT_H_REG, raw_data, 2);
